...
class td_qlearning:
    ...
    def __init__(self, directory):
        self.Q = {}          # Q-table
        self.rewards = {}    # cached rewards
        self.trials = []     # list of sequences of (state,action)
            ...
            if file.endswith(".csv"):
                ...
                with open(filepath, newline='') as csvfile:
                    ...
                    for row in reader:
                        ...
                        # Initialize Q(s,a) only for nonterminal actions
        ...
        for _ in range(self.max_iterations):
            ...
            for trial_seq in self.trials:
                for k in range(len(trial_seq) - 1):
                    ...
                    next_state, _ = trial_seq[k + 1]       # CORRECT FORWARD TRANSITION

                    if prev_action is None:
                        continue

                    old_q = self.Q.get((prev_state, prev_action), 0)
                    new_q = self.temporal_difference_qlearning(prev_state, prev_action, next_state)
    ...
    # ============================================================
    #  EXACT SCREENSHOT TD UPDATE
    # ============================================================
    def temporal_difference_qlearning(self, prev_state, prev_action, state):
        # 1. compute Q(s', a') for all possible actions
        actions = self.available_actions(state)
        action_qvalues = {}

        for action in actions:
            action_qvalues[action] = self.Q.get((state, action), 0)

        # 2. TD error term EXACTLY LIKE SCREENSHOT:
        #    reward(prev_state) + γ * max_a' Q(s',a') – Q(prev_state,prev_action)

        max_next_q = max(action_qvalues.values()) if action_qvalues else 0
        old_q = self.Q.get((prev_state, prev_action), 0)

        error_term = self.reward(prev_state) + self.gamma * max_next_q - old_q

        # 3. Update rule EXACTLY LIKE SCREENSHOT:
        #    Q = Q + α * error_term
        self.Q[(prev_state, prev_action)] = old_q + self.alpha * error_term

        # 4. Return optimal action argmax_a Q(s',a)
        if action_qvalues:
            optimal_action = max(action_qvalues, key=action_qvalues.get)
        else:
            optimal_action = 0
    ...
    def available_actions(self, state):
        ...
        return [i for i in [1, 2, 3] if i <= c_bag and c_bag > 0]
